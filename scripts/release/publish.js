...
async function main(tag, opts) {
	...
	let getReleaseByTagUrl = `https://api.github.com/repos/andrewiggins/publish-gh-release-package/releases/tags/${tag}`;
	let response = await fetch(getReleaseByTagUrl);
	...
	let release = /** @type {Release} */ (await response.json());
